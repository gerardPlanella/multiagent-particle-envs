...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world):
        ...
        redraw_2 = False
        redraw_3 = False
        while redraw_1 or redraw_2 or redraw_3:
            redraw_1 = False
            redraw_2 = False
            redraw_3 = False
            ...
            landmark_pts = [np.random.uniform(0.1, world.size - 0.1, size=2) for _ in range(self.num_landmarks)]

            # ensure predators not initialized on top of prey
            redraw_1 = overlaps(prey_pt, init_pts, world.size, threshold=0.5)
            redraw_2 = overlaps(prey_pt, landmark_pts, world.size, threshold = 0.1)
            for pt in init_pts:
                if redraw_3 == True:
                    print(f"Redraw: 1:{redraw_1}, 2:{redraw_2}, 3:{redraw_3}")
                    continue
                else:
                    redraw_3 = overlaps(pt, landmark_pts, world.size, threshold = 0.1)
            print(f"Redraw: 1:{redraw_1}, 2:{redraw_2}, 3:{redraw_3}")
        ...
        for i, landmark in enumerate(world.landmarks):
            if not landmark.boundary:
                landmark.state.p_pos = landmark_pts[i] % world.size
    ...
    def observation(self, agent, world):
        ...
        obs = np.concatenate([agent.state.p_pos] + other_pos + entity_pos)
        return obs
